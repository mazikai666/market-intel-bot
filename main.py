...
def choose_news() -> dict:
    ...
    if not picked:
        raise ValueError("没有抓到可用新闻，请检查 RSS 源。")
...
def build_html_report(data: dict, source_images: list[str]) -> str:
    ...
    watch_points_html = render_watch_points(data.get("watch_points", []))
...
<body>
  <div class="shell">
    ...
    <section class="split">
      <div class="card">
        <h2>三点看懂这条资讯</h2>
        <ul class="summary-list">{key_points_html}</ul>
      </div>
      <div class="card">
        <h2>为什么现在重要</h2>
        <div class="meta-block">
          <div class="meta-item"><div class="meta-label">Now in focus</div><div class="meta-value">{why_now}</div></div>
          <div class="meta-item"><div class="meta-label">Category</div><div class="meta-value">{escape(meta["label"])}</div></div>
        </div>
      </div>
    </section>
    ...
    <section class="content-grid">
      ...
      <div class="card">
        ...
        <div class="outlook-grid">
          <div class="mini"><h3>1天</h3><div>{outlook_1d}</div></div>
          <div class="mini"><h3>3天</h3><div>{outlook_3d}</div></div>
          <div class="mini"><h3>7天</h3><div>{outlook_7d}</div></div>
        </div>
      </div>
    </section>
